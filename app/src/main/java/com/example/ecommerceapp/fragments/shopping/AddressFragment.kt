...
import com.example.ecommerceapp.viewmodels.UserinfoViewModel
import dagger.hilt.android.AndroidEntryPoint
import kotlinx.coroutines.flow.collectLatest
@AndroidEntryPoint
class AddressFragment:Fragment() {
    private lateinit var binding: FragmentAddressBinding
    val viewModels  by viewModels<UserinfoViewModel>()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        lifecycleScope.launchWhenStarted {
            viewModels.addNewInfo.collectLatest {
                when(it){
                    is Resource.Loading ->{
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.apply {
            buttonSave.setOnClickListener {
                ...
                val userinfo = UserInfo(addressTitle,fullName,street,phone,city)

                viewModels.addAddress(userinfo)
            }
        }
    }
}